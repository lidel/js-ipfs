...
module.exports = {
  ...
  async handler (request, h) {
    ...
    // Content-Type sniffing over initial bytes
    const { stream, contentType } = await new Promise((resolve, reject) => {
      try {
        const catReadableStream = ipfs.catReadableStream(data.cid)
        peek(catReadableStream, 512, (err, chunk, stream) => {
          if (err) {
            log.error(err)
            return reject(err)
          }
    ...
    const res = h.response(stream.pipe(new ResponseStream()))
